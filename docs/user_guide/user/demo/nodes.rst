...
Note at this point we are building a Spheral ``NodeList`` object (a ``SolidNodeList`` to be precise), but we have actually not created any nodes yet.  This NodeList object is the object we will populate with points to represent the geometry and initial conditions of our problem to evolve from in a later section of the script (:ref:`point-generation`), but at this point we are simply building our choice of NodeList (which could be one of several types).
