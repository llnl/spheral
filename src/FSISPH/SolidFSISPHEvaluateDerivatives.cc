...
#pragma omp for
    for (auto kk = 0u; kk < numPairs; ++kk) {
      ...
      if (!decouple){
        ...
        // don't let the pressure be less than the min allowable
        const auto Peffi = max(Pdi,minPij);
        const auto Peffj = max(Pdj,minPij);

        // don't let deviatoric stress exceed min pairwise yield
...
//------------------------------------------------------------------------------
// Legacy Version of FSISPH
//------------------------------------------------------------------------------
template<typename Dimension>
template<typename QType>
void
SolidFSISPH<Dimension>::
secondDerivativesLoopLegacy(const typename Dimension::Scalar time,
                      const typename Dimension::Scalar dt,
                      const DataBase<Dimension>& dataBase,
                      const State<Dimension>& state,
                      StateDerivatives<Dimension>& derivs,
